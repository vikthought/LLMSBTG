#!/bin/bash
#SBATCH --job-name=lp6_p12
#SBATCH --output=logs/lp6_phase12_%j.out
#SBATCH --error=logs/lp6_phase12_%j.err
#SBATCH -p week
#SBATCH -t 24:00:00
...
# ===========================================================================
# 6-seed extended ablation — Phases 1 + 2 (CPU): probes + CKA
# ===========================================================================
#
# Part 2 of the 3-job chain.  No --gpus directive → no GPU allocated → the
# cluster's GPU-idle policy literally cannot apply.  Direct analog of
# cluster_lp3_phase12_cpu.sh, with 6 seeds and the timestamped models-dir
# convention.
#
...
#   sbatch --dependency=afterok:$PHASE0 --export=ALL,MODELS_DIR=... \
...
# folded in here for simplicity (~2h additional CPU work for 6 seeds).
#
# Output: results/lagpair_ablation_6seed/probes/probe_baselines_summary.json
#         results/lagpair_ablation_6seed/cka/cka_summary.json
# ===========================================================================
...
# ---------------------------------------------------------------------------
# Configuration — same OUT_DIR as Phase 0 / Phase 3.  MODELS_DIR resolution
# matches Phase 0's precedence chain (export → DEFAULT → newest auto-discover).
# ---------------------------------------------------------------------------
...
echo "============================================================"
echo " 6-seed ablation Phases 1+2 (CPU): probes + CKA"
...
if [ ! -f "$SUMMARY_PATH" ]; then
    ...
    echo "       sbatch --export=ALL,MODELS_DIR=$MODELS_DIR \\" >&2
    echo "              scripts/cluster_lp6_phase0_gpu.sh" >&2
    exit 1
fi
...
echo "             scripts/cluster_lp6_phase3_gpu.sh"
echo "============================================================"
